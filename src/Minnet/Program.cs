...
using Microsoft.IO;
using Minnet.Api;
using Minnet.Api.Controllers;
using Minnet.Api.Middlewares;
using Minnet.Api.Responses;
using Minnet.Configuration;
using Minnet.Crypto.Hashing.Algorithms;
using Minnet.Crypto.Hashing.Equihash;
using Minnet.Crypto.Hashing.Ethash;
using Minnet.Extensions;
using Minnet.Messaging;
using Minnet.Mining;
using Minnet.Native;
using Minnet.Notifications;
using Minnet.Payments;
using Minnet.Persistence;
using Minnet.Persistence.Dummy;
using Minnet.Persistence.Postgres;
using Minnet.Persistence.Postgres.Repositories;
using Minnet.Util;
...
[assembly: InternalsVisibleToAttribute("Minnet.Tests")]
...
public class Program : BackgroundService
{
    public static async Task Main(string[] args)
    {
        ...
        catch(PoolStartupException ex)
        {
            ...
            await Console.Error.WriteLineAsync("\nThe cluster failed to start and cannot proceed.");
        }
        ...
        catch(AggregateException ex)
        {
            ...
            await Console.Error.WriteLineAsync("The cluster failed to start and cannot proceed.");
        }
        ...
        await Guard(()=> Task.WhenAll(tasks), ex =>
        {
            switch(ex)
            {
                case PoolStartupException pse:
                {
                    ...
                    logger.Error(() => "The cluster failed to start and cannot proceed.");
    ...
    private static ClusterConfig ReadConfig(string file)
    {
        try
        {
            Console.WriteLine($"The software is using the following configuration file: '{file}'");
        ...
        {
            throw new PoolStartupException($"We have encountered an error with the configuration file: {ex.Message}");
        }

        catch(JsonSerializationException ex)
        {
            throw new PoolStartupException($"CWe have encountered an error with the configuration file: {ex.Message}");
        }
    ...
    private static void ValidateRuntimeEnvironment()
    {
        ...
        if(!RuntimeInformation.IsOSPlatform(OSPlatform.Windows) && Environment.UserName == "root")
            logger.Warn(() => "We highly recommend against running the software as root for security reasons. Please use a regular user account for optimal safety.");

        // require 64-bit on Windows
        if(RuntimeInformation.IsOSPlatform(OSPlatform.Windows) && RuntimeInformation.ProcessArchitecture == Architecture.X86)
            throw new PoolStartupException("To run Minnet, make sure you have a 64-Bit version of Windows installed on your system.");
    }
...
\____|__  /__|___|  /___|  /\___  >__|  
        \/        \/     \/     \/      
");
        Console.WriteLine(" High-performance mining software with multiple pool and currency support.");
        Console.WriteLine();
    }
    ...
    private static async Task ConfigurePostgresCompatibilityOptions(IServiceProvider services)
    {
        ...
        if(!clusterConfig.Persistence.Postgres.EnableLegacyTimestamps.HasValue)
        {
            ...
            if(columnType != null)
                ...
                logger.Warn(() => "We encountered an issue with the auto-detection for Npgsql Legacy Timestamp Behavior. To fix this, we recommend setting 'EnableLegacyTimestamps' in your Minnet Database configuration to 'true' or 'false'. This will ensure seamless operation of the software without any problems.");
        }
    ...
    private static void ConfigurePersistence(ContainerBuilder builder)
    {
        if(clusterConfig.Persistence == null &&
           ...
           clusterConfig.ShareRelay == null)
            throw new PoolStartupException("Please note that persistence is not configured.");
